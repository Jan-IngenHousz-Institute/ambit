...
| 2026-09-03 | **openJII generic command object + `INFO`** — `openjii_proto` answers `{"command":"<name>"[,"params":"…"]}` with one bare `{"status":"success","data":…}` / `{"status":"error","error":"…"}` line (no `7A1E3AA1`); `frontend_json` registers `INFO` (device_name, device_type `ambit`, device_id = eFuse MAC, firmware_version, capabilities). This is the app's second identification probe (`identify-device.ts`); before, it fell into the envelope writer and the app bound a raw generic connector whenever the `hello` probe was polluted (boot banner after a DTR/RTS reset on port open, `BAD COMMAND` after a sleep wake). Binary path, router, envelope shapes untouched. | Text/JSON path only (no ambyte HW needed for the change itself; re-run weak A for hygiene) | ☐ pending HW — (1) terminal: `{"command":"INFO"}\n` → exactly one line, `JSON.parse`-able, `status=success`, `data.device_type=ambit`, `data.device_id` equals the trace `sensor_id`; (2) `{"command":"nope"}\n` → one `status=error` line; (3) `[{"label":"hello"}]\n` still returns the envelope + frame token (no collision); (4) Android openJII app over the USB-C bridge: plug in cold, grant permission, confirm the connection log says family `ambit` (not `generic`) and a measurement runs; repeat after a `reboot` and after 30 s idle; (5) Web Serial (desktop Chrome) same as (4). |
