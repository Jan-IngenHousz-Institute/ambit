...
| 2026-09-03 | **openJII generic command object + `INFO`** — `openjii_proto` answers `{"command":"<name>"[,"params":"…"]}` with one bare `{"status":"success","data":…}` / `{"status":"error","error":"…"}` line (no `7A1E3AA1`); `frontend_json` registers `INFO` (device_name, device_type `ambit`, device_id = eFuse MAC, firmware_version, capabilities). This is the app's second identification probe (`identify-device.ts`); before, it fell into the envelope writer and the app bound a raw generic connector whenever the `hello` probe was polluted (boot banner after a DTR/RTS reset on port open, `BAD COMMAND` after a sleep wake). Also (`cc5d18e`) the console token now ends at `\n` (`Serial_Input_Chars(choose, ":,\n", …)`): it was framed by 200 ms of silence alone, so a second line inside that window was glued onto the first (`hello\r\n` + `{"command":"INFO"}\n` → token `hello{"command"` → BAD COMMAND, JSON never reached the envelope router; same for the ambyte `uart_query` priming newline, which is why no JSON ever answered through that tool on 1.1.5). Binary path, router branches, envelope shapes untouched. | Text/JSON path + weak A (binary ping/info/temp/spec re-driven) | ✅ PASSED 2026-09-03 on two AMBITs (AD81 `3C:DC:75:0D:FD:20`, AD89 `3C:DC:75:0E:02:B8`) OTA'd via the ambyte (`ambit_ota all http://…/firmware.bin`, image confirmed, rollback cancelled), driven from the ambyte console over the FFC after `lua stop` + `ambit_dl <ch> run`: `hello` → `NEW AD81 Ready FW:1.1.4-5-gcc5d18e`; `{"command":"INFO"}` → one 142-byte `{"status":"success","data":{"device_name":"AD81","device_type":"ambit","device_id":"3C:DC:75:0D:FD:20","firmware_version":"1.1.4-5-gcc5d18e"}}`; `{"command":"nope"}` → one `status=error` line; `[{"label":"hello"}]` → envelope + `7A1E3AA1`; text `temp` → 3 tab floats; binary `ping_uart`, `ambit_info <ch> 2` (48 B, MAC, size), `ambit_temp`, `ambit_spec` OK; Lua MPF/SS measurements stored after the OTA. Control: the same JSON queries on the still-1.1.5 unit timed out (only `hello` answered). Caveat learned: after binary traffic the AMBIT is BINARY-latched and light-sleeps; the ambyte text tool's single priming newline does not reliably wake it (the wake eats the in-flight line), so reset the unit (`ambit_dl <ch> run`) or use the binary wake before text queries — frozen behaviour, not a regression. Still ☐: (4) Android openJII app over the USB-C bridge: cold plug → family `ambit` (not `generic`), measurement runs; repeat after `reboot` and after 30 s idle; (5) desktop Chrome Web Serial same. |
