...
int run_arr_type1(uint8_t length, uint8_t* arr, bool led_persist, bool allow_interrupt, bool json_output, bool retain){
  ...
  if (pam_finish_results(d_env, d_fluor, d_fluoRef, d_sun, d_leaf, d_730, d_730Ref, d_timing, d_env_time,
                         length, arr, subsampling, data_count[2] > 0, allow_interrupt,
...
int run_arr_trigger_validate(uint8_t length, uint8_t* arr){
  return ambit_trace_v3::validate_run_protocol(arr, length, MAX_DATACLASS_SIZE - 1U, NULL)
      ? ARR_TRIG_OK : ARR_TRIG_BAD_LINE;
}
...
int run_arr_trigger(uint8_t length, uint8_t* arr, bool led_persist, bool allow_interrupt,
  ...
  dataclass *d_sun = buffers.d_sun;
  dataclass *d_leaf = buffers.d_leaf;
  dataclass *d_730 = buffers.d_730;
